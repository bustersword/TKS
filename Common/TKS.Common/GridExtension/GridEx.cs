...
namespace TKS.Common
{
    ...
    public static class GridEx
    {
        ...
        /// <summary>
        /// 对容器内绑定验证属性的控件进行验证
        /// <para>支持TextBox、DatePicker、ComboBox</para>
        /// </summary>
        /// <param name="grid"></param>
        public static void ValidateItems(Grid grid)
        {
            error = 0;
            List<Control> children = GetChildObjects<Control>(grid, null);
            ...
            {
                BindingExpression express = null ;
                if (item.GetType().Name == ControlType._TextBox)
                {
                    TextBox t = item as TextBox;
                    express = t.GetBindingExpression(TextBox.TextProperty);
                }
                else if (item.GetType().Name == ControlType._ComboBox)
                {
                    ComboBox t = item as ComboBox;
                    express = t.GetBindingExpression(ComboBox.SelectedValueProperty);
                }
                else if (item.GetType().Name == ControlType._DatePicker)
                {
                    DatePicker t = item as DatePicker;
                    express = t.GetBindingExpression(DatePicker.TextProperty);
                }
        ...
        public static void SetValue(Grid grid, object value, GridExSourceType type)
        {
            _type = type;
        ...
        static int error = 0;
        static GridExSourceType _type = GridExSourceType.DTO;
        /// <summary>
        /// 是否验证通过
        /// </summary>
        ...
        static void SetControlValue(Control control, object value)
        {
            ...
            //数据实体对容器内控件赋值
            if (_type == GridExSourceType.DataSource)
                controlset.SetValue(control, value);
            else
                //数据源对容器内控件赋值（区别于数据实体DTO）
                controlset.SetValueUseDTO(control, value);
        }
        ...
        interface ISetControlVal
        {
            ...
            bool SetValueUseDTO(object obj, object val);
        ...
        class TextBoxValOP : ISetControlVal
        {
            ...
            public bool SetValueUseDTO(object obj, object val)
            {
                return SetValue(obj, val);
            }
        ...
        class ComboBoxValOP : ISetControlVal
        {
            ...
            public bool SetValueUseDTO(object obj, object val)
            {
                ...
                if (comboval == null)
                {
                    throw new Exception(obj.GetType().Name + "ComboBox的赋值数据源非字符串类型");
                }

                comboBox.SelectedValue = comboval;
        ...
        class DatePickerValOP : ISetControlVal
        {
            ...
            public bool SetValue(object obj, object val)
            {
                ...
                datePick.Text = date.ToString("yyyy-MM-dd HH:mm:ss");
                flag = true;
                return flag;
        ...
        class ListBoxValOP : ISetControlVal
        {
            ...
            public bool SetValueUseDTO(object obj, object val)
            {
                ...
                listbox.SelectedValue = itemsVal;
                flag = true;
                return flag;
            }
        }
